...
import ru.oftendev.itsmymeta.meta.getMeta
import ru.oftendev.itsmymeta.meta.getOfPlayer

class InfoCommand(plugin: ItsMyMeta): Subcommand(
    plugin,
    "info",
    "itsmymeta.info",
    false
    ...
    override fun onExecute(sender: CommandSender, args: MutableList<String>) {
        ...
        sender.sendMessage(plugin.langYml.getFormattedString("messages.prefix") +
                plugin.langYml.getString("messages.meta-info")
            .replace("%player%", player.savedDisplayName)
            .replace("%meta%", meta.id)
            .replace("%value%", player.getMeta(meta).toNiceString())
