...
class TakeCommand(plugin: ItsMyMeta): Subcommand(
    plugin,
    "take",
    "itsmymeta.take",
    false
    ...
    override fun onExecute(sender: CommandSender, args: MutableList<String>) {
        ...
        when(meta.type) {
            StatType.STRING -> player.takeMeta(meta, metaValue)
            StatType.INTEGER -> player.takeMeta(meta, metaValue.toIntOrNull() ?: kotlin.run {
                sender.sendMessage(plugin.langYml.getMessage("invalid-value"))
                return
            })
            StatType.DOUBLE -> player.takeMeta(meta, metaValue.toDoubleOrNull() ?: kotlin.run {
                sender.sendMessage(plugin.langYml.getMessage("invalid-value"))
                return
            })
        }
        ...
        sender.sendMessage(plugin.langYml.getFormattedString("messages.prefix") +
                plugin.langYml.getString("messages.meta-taken")
